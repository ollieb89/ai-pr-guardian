...
exports.detectAiGenerated = detectAiGenerated;
...
function detectAiGenerated(files, commits, totalAdditions, totalDeletions) {
    const signals = [];
    let confidence = 0;
    let totalLines = 0;
    ...
    const genericCommits = commits.filter(c => GENERIC_COMMIT_PATTERNS.some(p => { p.lastIndex = 0; return p.test(c.message.trim().split('\n')[0]); }));
