export function formatReport(score, detection, prAuthor, threshold) {
    ...
    if (score.total < threshold) {
        lines.push('### ⚠️ Quality Gate Failed', '', 'This PR scored **' + score.total + '/100**, below the configured threshold of **' + threshold + '**.', '', '**Suggested improvements:**');
        if (score.testCoverageDelta < 50)
            lines.push('- Add tests for new code');
        if (score.commitMessageQuality < 50)
            lines.push('- Improve commit messages (use Conventional Commits format)');
        if (score.codeDuplication < 50)
            lines.push('- Reduce code duplication');
        if (score.fileScatter < 50)
            lines.push('- Consider splitting this PR into smaller, focused changes');
        if (score.documentation < 50)
            lines.push('- Update documentation for new features');
        if (score.boilerplateRatio < 50)
            lines.push('- Remove placeholder/TODO code before submitting');
        lines.push('');
    }
    else {
